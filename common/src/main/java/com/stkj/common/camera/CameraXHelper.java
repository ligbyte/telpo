...
import com.stkj.common.utils.CameraUtils;
...
public class CameraXHelper extends ActivityWeakRefHolder {
    ...
    public void prepare(PreviewView previewView, boolean isFrontCamera) {
        ...
        previewOrientation = CameraUtils.getCameraPreviewOrientation(activityWithCheck, isFrontCamera);
        if (displayOrientation == -1) {
            displayOrientation = activityWithCheck.getWindowManager().getDefaultDisplay().getRotation();
        }
    ...
    public void startPreview() {
        try {
            if (processCameraProvider != null) {
                ...
                CameraSelector cameraSelector = cameraSelectorBuilder.requireLensFacing(isFrontCamera ? CameraSelector.LENS_FACING_FRONT : CameraSelector.LENS_FACING_BACK).build();
                preview = new Preview.Builder().build();
                preview.setTargetRotation(displayOrientation);
                preview.setSurfaceProvider(previewView.getSurfaceProvider());
                processCameraProvider.bindToLifecycle((LifecycleOwner) getHolderActivity(), cameraSelector, preview);
                isPrepared = true;
                isFirstInit = false;
                if (onCameraHelperCallback != null) {
                    onCameraHelperCallback.onPreviewSuccess();
                }
            }
            ...
            if (onCameraHelperCallback != null) {
                onCameraHelperCallback.onPreviewError(e.getMessage());
            }
        }
    ...
    public void takePicture() {
        ...
        try {
            if (processCameraProvider != null) {
                ...
                imageCapture.setTargetRotation(displayOrientation);
    ...
    public interface OnCameraHelperCallback {
        default void onImageAnalyze(ImageProxy imageProxy) {

        }

        default void onPreviewError(String message) {

        }

        default void onPreviewSuccess() {
