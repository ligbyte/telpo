package com.stkj.common.ui.adapter;
...
import com.stkj.common.ui.adapter.data.CommonAdapterDataHelper;
import com.stkj.common.ui.adapter.holder.CommonRecyclerViewHolder;
import com.stkj.common.ui.adapter.holder.ItemViewTypeHolder;
...
import com.stkj.common.ui.adapter.holder.placeholder.PlaceViewTypeHolder;
import com.stkj.common.ui.adapter.holder.unknownholder.UnknownViewHolder;
...
public class CommonRecyclerAdapter extends RecyclerView.Adapter<CommonRecyclerViewHolder> {
    ...
    public void addFirstLayout(@LayoutRes int layoutResId) {
        PlaceRecyclerViewHolder.Factory factory = new PlaceRecyclerViewHolder.Factory(layoutResId);
        addFirstLayout(layoutResId, factory);
    }

    public void addFirstLayout(@LayoutRes int layoutResId, @NonNull PlaceRecyclerViewHolder.Factory factory) {
        if (mPlaceVHFactorySA == null) {
            mPlaceVHFactorySA = new SparseArray<>();
        }
        mPlaceVHFactorySA.put(layoutResId, factory);
        ...
        addFirstData(new PlaceModel(layoutResId));
    }

    public void removeLastData() {
        if (!mItemDataList.isEmpty()) {
            int lastIndex = mItemDataList.size() - 1;
            Object obj = mItemDataList.get(lastIndex);
            mItemDataList.remove(lastIndex);
            notifyItemRemoved(lastIndex);
            for (OnItemEventListener eventListener : mItemEventListenerList) {
                eventListener.onRemoveItemData(obj);
            }
            Log.d("CommonRecyclerAdapter", "removeFirstData");
        }
    }

    public void removeFirstData() {
        if (!mItemDataList.isEmpty()) {
            Object obj = mItemDataList.get(0);
            mItemDataList.remove(0);
            notifyItemRemoved(0);
            for (OnItemEventListener eventListener : mItemEventListenerList) {
                eventListener.onRemoveItemData(obj);
            }
    ...
    public void addFirstData(Object obj) {
        if (obj != null) {
            mItemDataList.add(0, obj);
            notifyItemInserted(0);
            for (OnItemEventListener eventListener : mItemEventListenerList) {
                eventListener.onAddItemData(obj);
            }
            Log.d("CommonRecyclerAdapter", "addFirstData");
        }
    }
    ...
    public void notifyChangeItemData(Object obj) {
        if (obj != null) {
