...
import com.stkj.supermarketmini.base.excel.annotation.ExcelReadAggregate;
...
public class ParserImpl<T> implements IParser<T> {
    ...
    public void doParse(Workbook workbook, ITabContext context, Class<T> tClass, IParseListener<T> listener) {
        ...
        for (int row = 1; row < physicalNumberOfRows; row++) {
            Row sheetAtOneRow = sheetAtOne.getRow(row);
            for (Cell cell : sheetAtOneRow) {
                cell.setCellType(Cell.CELL_TYPE_STRING);
            }
            parseData(sheetAtOneRow, context.getExcelProperty(), tClass, listener);
        }
