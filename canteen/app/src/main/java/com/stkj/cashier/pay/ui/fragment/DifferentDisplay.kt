...
import com.stkj.cashier.eventbus.MessageEventType
import com.stkj.cashier.greendao.biz.CompanyMemberBiz
import com.stkj.cashier.pay.data.PayConstants
import com.stkj.cashier.pay.helper.CommonTipsHelper
import com.stkj.cashier.pay.helper.SystemEventHelper
import com.stkj.cashier.pay.model.ConsumeRefundListBean
import com.stkj.cashier.pay.model.FacePassRecogMode
import com.stkj.cashier.pay.model.FacePassTrackResult
import com.stkj.cashier.pay.model.MessageEventBean
import com.stkj.cashier.pay.ui.adapter.ConsumeRefundListAdapter
...
import com.stkj.cashier.utils.camera.CameraPreview
import com.stkj.cashier.utils.camera.CameraPreviewData
import com.stkj.cashier.utils.camera.ComplexFrameHelper
...
import com.stkj.common.utils.ThreadUtils.runOnUiThread
...
//        tvWifiState = findViewById<TextView>(R.id.tvWifiState)
        ...
        val switchFacePassPay = PaymentSettingMMKV.getSwitchFacePassPay()
    ...
    override fun dispatchKeyEvent(event: KeyEvent): Boolean {
        if (MainApplication.mMainActivity!=null){
            return MainApplication.mMainActivity!!.dispatchKeyEvent(event)
        }
    ...
    inner class FeedFrameThread : Thread() {
        ...
        override fun run() {
            ...
            while (!isInterrupt) {
                ...
                try {
                    if (MainApplication.cameraType == FacePassCameraType.FACEPASS_DUALCAM) {
                        ...
                        detectionResult = MainApplication.mFacePassHandler?.feedFrameRGBIR(imageRGB, imageIR)
                    } else {
                        ...
                        try {
                            ...
                            detectionResult = MainApplication.mFacePassHandler?.feedFrame(imageRGB)
                ...
                if (detectionResult != null && detectionResult.message.isNotEmpty()) {
                    ...
                    for (i in detectionResult.images.indices) {
                        if (detectionResult.images[i].rcAttr.respiratorType != FacePassRCAttribute.FacePassRespiratorType.NO_RESPIRATOR) {
                            ...
                            trackOpts[i] =
                                FacePassTrackOptions(detectionResult.images[i].trackId, -1f, -1f,-1f,-1f)
                        }
                    }
            ...
            while (!isInterrupt) {
                ...
                try {
                    ...
                    if (isStartFaceScan.get()) {
                        ...
                        recognizeResult = MainApplication.mFacePassHandler?.recognize(
...
//                                if (recognizeResult != null && recognizeResult.length > 0) {
                        ...
                        if (recognizeResult != null && recognizeResult.isNotEmpty()) {
                            for (result in recognizeResult) {
                                ...
                                if (MainApplication.mFacePassHandler != null) {
                                    faceTokens =
                                        MainApplication.mFacePassHandler!!.getLocalGroupInfo(PayConstants.GROUP_NAME)
                                }
                               ...
                               // CompanyMemberBiz.getCompanyMember2()
                                ...
                                if (faceToken.isNotEmpty() && FacePassRecognitionState.RECOGNITION_PASS == result.recognitionState) {
                                    ...
                                    if (companyMember != null) {
                                        ...
                                        try {
                                            val b = MainApplication.mFacePassHandler!!.deleteFace(
                                                faceToken.toByteArray(StandardCharsets.ISO_8859_1)
                                            )
                                            //重新识别
                                            MainApplication.mFacePassHandler?.reset()
...
//                        }
    ...
    private fun handleFacePassFailRetryDelay() {
        if (canSpeakFacePassFail) {
            ...
            Observable.timer(3, TimeUnit.SECONDS)
                ...
                .subscribe(canSpeakFacePassFailObserver as DefaultDisposeObserver<Long>)
            tvFaceTips2.visibility = View.VISIBLE
    ...
    private fun showSuccessFace(imageUrl:String){
        if (ivSuccessHeader != null) {
            ...
            GlideApp.with(MainApplication.mainApplication)
                .load(imageUrl)
...
//                    tvCompanyName.text = it
            ...
            MessageEventType.AmountRefundList -> {
                ...
                message.content?.let {
                    val fromJson = Gson().fromJson(
                        it,
                        ConsumeRefundListBean::class.java
                    )
                    if (fromJson.results != null && fromJson.results!!.size > 0) {
                        ...
                        Glide.with(MainApplication.mainApplication).load(fromJson.customerImg)
                            .placeholder(R.mipmap.icon_camerapreview_person_3) // 设置占位图
                            .into(ivHeader2)
                        layoutManager = LinearLayoutManager(MainApplication.mainApplication);//添加布局管理器
    ...
    fun ttsSpeak(value: String) {
        MainApplication.TTS.setSpeechRate(1f)
        MainApplication.TTS.speak(
            value,
