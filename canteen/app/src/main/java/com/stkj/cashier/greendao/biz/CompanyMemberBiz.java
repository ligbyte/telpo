...
public class CompanyMemberBiz {
    ...
    public final static String TAG = "CompanyMemberBiz";
    ...
    public static void addCompanyMembers(List<CompanyMemberdbEntity> entities){
        ...
        for (int i = 0; i < entities.size(); i++) {
            ...
            if (unique == null) {
                ...
                if(MainApplication.mFacePassHandler !=null&&unique.getFaceToken()!=null){
                    try {
                        boolean b = MainApplication.mFacePassHandler.deleteFace(unique.getFaceToken().getBytes(StandardCharsets.ISO_8859_1));
                        Log.d("删除地库人脸token成功" , unique.getFullName()+"=="+b);
                    } catch (Exception e) {
                        e.printStackTrace();
                        Log.e("删除地库人脸token失败",unique.getFullName()+e.getMessage());
                    }
                }
...
//        entity.setIsDelete(true);
    ...
    public static void getCompanyMember2(){
        ...
        Log.e(TAG,"本地数据库人脸 list.size() : " +list.size());

        for (int i = 0; i < list.size(); i++) {
            CompanyMemberdbEntity companyMemberdbEntity = list.get(i);
             Log.e(TAG,"showConfirmDialog"+companyMemberdbEntity.getFullName()+"/"+companyMemberdbEntity.getUserNumber()+"/"+companyMemberdbEntity.getFaceToken());
        }
